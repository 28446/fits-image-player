...
VERSION: 1.0.0092
FIXES:
- Grid Aliasing: Replaced while-loop with index-based range to prevent skipped lines.
- Rendering: Forced non-antialiased pen for razor-sharp grid lines.
- Video: Burned-in grid logic for WebM remains consistent with display.
- UI: 100% English interface and internal documentation.
"""
...
def inspect_fits_metadata_safely(file_path):
    """Extracts Bayer pattern and color mode from FITS header."""
...
def execute_debayer_safely(mono, pat):
    """Converts RAW bayer data into BGR image using OpenCV."""
    try:
        p = str(pat).upper()
...
def apply_pro_stretch(img_float, low_p, high_p, mid_p, is_rgb):
    """Calculates non-linear stretch with median-based channel balancing."""
...
def draw_histogram_from_data(img8, width=180, height=45, save_path=None):
    """Renders a small histogram visualization for the UI overlay."""
...
def scan_for_lights_directories(root_path):
    """Scans subdirectories for folders named 'Lights'."""
    dirs = []
    try:
...
def gather_and_validate_fits_multi(directory_list):
    """Returns a list of all .fits/.fit files in the provided folders."""
    valid = []
...
def find_best_siril_seq(dir_path):
    """Locates the registration sequence file generated by Siril."""
    try:
...
def parse_siril_seq_data(seq_path):
    """Parses FWHM/HFR star analysis data from Siril .seq files."""
...
def format_coords(ra_val, dec_val):
    """Converts raw RA/DEC degrees into formatted HMS/DMS strings."""
...
class AstroViewPlayerWindow(QMainWindow):
    ...
    def prepare_f(self):
        ...
        # 2. Save to Video with "Burned-in" Grid (Logic for file output)
    ...
    def on_paint(self, event):
        ...
        # ALIASING-FREE DISPLAY GRID:
        # We calculate the number of steps based on integer range to prevent decimal accumulation (skipped lines).
        if self.grid_en and self.grid_st > 5:
            p.setRenderHint(QPainter.RenderHint.Antialiasing, False) # Sharp lines
            ...
            for i in range(num_v + 1):
                ...
                if lx <= self.img_rect.right():
                    p.drawLine(lx, self.img_rect.top(), lx, self.img_rect.bottom())
            ...
            for j in range(num_h + 1):
                ...
                if ly <= self.img_rect.bottom():
                    p.drawLine(self.img_rect.left(), ly, self.img_rect.right(), ly)
...
def _process_single_frame_worker(args):
    ...
    try:
        with fits.open(src_p, memmap=False) as hd:
            ...
            final_rgb = rgb; final_bayer = bayer
            if bayer == "AUTO":
            ...
            if not img_ready:
                ...
                img_processed = apply_pro_stretch(img_f, low, high, mid, final_rgb)
...
class FITSImagePlayerGui(QMainWindow):
    ...
    def on_action(self):
        ...
        if self.btn_act.text() == "Play Video":
            reg_p = os.path.join(self.cache_dir, "datasets_registry.json")
            if not os.path.exists(reg_p): self.check_state(); return
            with open(reg_p, "r") as f:
                ...
                for pf in ff:
                    ...
                    m = entry["meta_item"].copy()
                    m["telemetry_active"] = self.chk_tele.isChecked()
                    m["hist_enabled"] = self.chk_hist.isChecked()
    ...
    def on_fin(self, p, m, el, rgb):
        ...
        if not self.worker.stop_requested and p:
            ...
            active = [k for k, v in self.chk_dirs.items() if v.isChecked()]
            sp = find_best_siril_seq(active[0])
            ph = generate_params_hash(self.current_low, self.current_high, self.current_mid, self.combo_bayer.currentText(), rgb, round(self.spin_scale.value()*0.01, 2), bool(sp), self.chk_hist.isChecked(), self.chk_tele.isChecked())

            for i in range(len(p)):
                fn = m[i]["filename"]
                reg[fn] = {"png_path": p[i], "param_hash": ph, "meta_item": m[i]}
